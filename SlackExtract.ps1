function Invoke-SlackExtract{
 <#
    ...
    [Optional] The Channel IDs that you want to extract messages and files for. Channel IDs generally start with a "C","G" or "D". If you do not specify ChannelIds, the script will download messages and files from ALL channels. You can determine the channel ID by inspecting the URL in the browser (e.g. https://slackextract.slack.com/messages/C024THD2R/ would have  a channel ID of C024GHP2R) 
...
function getSlackToken {
    ...
    $index = $response.toString().IndexOf("api_token: `"xoxs-")
    if($index -ge 0){
        $subString1 =$response.toString().Substring($index, 150)
        $SlackToken = $subString1.remove(0,12).split("`"")[0]
        Write-Host -ForegroundColor Cyan $SlackToken
        return $SlackToken
    }
